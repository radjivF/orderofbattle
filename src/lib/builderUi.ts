...
/** Format points as "spent pts" for battle record picker and score identity. */
export function formatArmyPoints(input: {
  ...
  return `${formatPoints(input.pointsSpent)}pts`;
}
...
export const LIBRARY_CARD_CLASS =
  "library-card parchment-card relative grid h-full min-h-[8.5rem] cursor-pointer grid-cols-[minmax(0,1fr)_7.5rem] overflow-hidden rounded-2xl text-parchment-ink sm:grid-cols-[minmax(0,1fr)_9.5rem]";
